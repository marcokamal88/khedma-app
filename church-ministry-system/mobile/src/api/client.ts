...
const HOST = Platform.OS === "android" ? "10.89.219.234" : "localhost";
...
apiClient.interceptors.request.use(async (config) => {
  ...
  console.log(
    `[API REQ] ${config.method?.toUpperCase()} ${config.baseURL}${config.url} | churchId: ${churchId || "default"} | hasToken: ${!!token}`,
  );
  return config;
});
...
apiClient.interceptors.response.use(
  (response) => {
    console.log(
      `[API RES] ${response.config.method?.toUpperCase()} ${response.config.url} | status: ${response.status}`,
    );
  ...
  async (error) => {
    if (error.response) {
      console.error(
        `[API ERR] ${error.config?.method?.toUpperCase()} ${error.config?.url} | status: ${error.response.status} | data:`,
        error.response.data,
      );
    } else if (error.request) {
      console.error(
        `[API ERR] No response received | ${error.config?.url} | message: ${error.message}`,
      );
    } else {
