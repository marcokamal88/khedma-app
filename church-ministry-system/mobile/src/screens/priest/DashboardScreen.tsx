...
export default function PriestDashboard({ navigation }: any) {
  const { t } = useLocale();
      ...
      <ScrollView style={styles.scroll} bounces={false}>
          ...
          <View style={styles.contextCard}>
            <View style={styles.contextRow}>
              <Text style={styles.contextLabel} numberOfLines={1}>
                {activeContext?.displayLabel || t('roles.priest')}
              </Text>
            </View>
          </View>
        </AppHeader>
        ...
        <View style={styles.content}>
          <View style={styles.metricsGrid}>
            <MetricCard value={`${stats?.totalSectors ?? 0}`} label={t('home.sectors')} bgColor="#e8f5e9" />
            <MetricCard value={`${stats?.totalStudents ?? 0}`} label={t('home.students')} bgColor="#fff3e0" />
            <MetricCard value={`${stats?.totalOpenTasks ?? 0}`} label={t('home.openTasks')} bgColor="#fce4ec" />
          </View>
        </View>
